    ...
    <View className="flex-1 bg-gray-100">
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <View className="bg-white p-6 shadow-md mb-4">
          <Text className="text-2xl font-bold text-gray-900 mb-2">{detail.title}</Text>
        ...
        <View className="bg-white p-4 rounded-lg shadow-md mb-4 mx-4">
          <Text className="text-gray-700 text-base mb-4">
            이 프로젝트는 심장질환 예측을 목표로 하며, Django와 Python을 활용하여 모델을 구축합니다.
            프로젝트는 6개월간 진행되며, 매주 스프린트를 통해 진행 상황을 공유합니다.
          </Text>
          <View className="flex-row justify-between items-center">
            <Text className="text-gray-700 text-base">모집 인원: {detail.current}/{detail.total}명</Text>
        ...
        <View className="bg-white p-4 rounded-lg shadow-md mx-4 mb-4">
          <View className="flex-row justify-between items-center mb-4">
            ...
            <Text className="text-xs text-gray-500">{dummyComments.length}개의 댓글</Text>
          </View>
          {dummyComments.map((comment) => (
            <View key={comment.id} className="border-b border-gray-200 pb-2 mb-2">
              <Text className="text-gray-800 font-semibold">{comment.author}</Text>
        ...
        {/* 비슷한 기술 스택 프로젝트 섹션 */}
        <View className="bg-white p-4 rounded-lg shadow-md mx-4 mb-4">
          <Text className="text-lg font-bold text-gray-900 mb-4">비슷한 기술 스택의 프로젝트</Text>
          {similarProjects.map((project) => (
            <View key={project.id} className="border-b border-gray-200 pb-2 mb-2">
              <Text className="text-gray-900 font-semibold">{project.title}</Text>
