...
const AllMenuScreen = ({ goToPage }: { goToPage: (page: string) => void }) => {
  const menuItems = [
    ...
    { label: "내가 지원한 공고", page: "MyApplications", icon: "clipboard", color: "#9C27B0" },
    { label: "알림", page: "Notifications", icon: "bell", color: "#F44336" },
    { label: "즐겨찾기", page: "Favorites", icon: "heart", color: "#E91E63" },
    ...
    >
      ...
      {menuItems.map((item, index) => (
        ...
        >
          ...
          <Text className="text-lg text-gray-800 font-medium">{item.label}</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
